...
const errorMessages_1 = require("./environment/errorMessages");
const routeConstants_1 = require("./lib/routeConstants");
const environment_1 = require("./environment/environment");
const routeGuard_1 = require("./lib/routeGuard");
...
class App {
    constructor() {
        ...
        this.logger = new (winston.Logger)({
            transports: [
                ...
                new (winston.transports.File)({
                    filename: `${environment_1.environment.logDirectory}/atmdev.log`,
                    timestamp: tsFormat,
        ...
        this.guard = new routeGuard_1.RouteGuard();
        this.mountAtmRoutes();
    }
    mountAtmRoutes() {
        ...
        */
        atmPublic.use((req, resp, next) => {
            ...
            var apiKey = req.headers['bbank-apikey'];
            ...
            if (apiKey != environment_1.environment.apiKey) {
                ...
                resp.json(errorMessages_1.errorMessages.INVALID_API_KEY);
            }
            else {
        ...
        atmPublic.get(routeConstants_1.routeConstants.ALIVE, (req, resp) => {
            resp.json(errorMessages_1.errorMessages.OK_MESSAGE);
        });
        atmPublic.get(routeConstants_1.routeConstants.AUTHENTICATE, (req, resp) => {
            this.atm.findAccountPin(req.params.acct, req.params.pin).then(result => {
                ...
                */
                ...
                resp.json(errorMessages_1.errorMessages.ACCOUNT_NOT_FOUND);
            });
        });
        /*
        ...
        */
        atmRouter.use((req, resp, next) => {
            //We are retrieving the values from the headers for each request
            var token = req.headers['bbank-secure'];
                ...
                if (this.guard.verifyToken(token)) {
                    ...
                    */
                    ...
                    if (decodedValue.acct = acct) {
                        this.logger.debug("Decoded account in token ", decodedValue.acct, " is not equal to the one sent by the parameter ", req.params.acct);
                        return resp.status(403).send(errorMessages_1.errorMessages.ACCOUNTS_DONT_MATCH);
                    }
                    else {
                        /*
                    ...
                    return resp.status(403).send(errorMessages_1.errorMessages.TOKEN_NOT_VALID);
                }
            }
            else {
                // if there is no token return an error
                return resp.status(403).send(errorMessages_1.errorMessages.NO_TOKEN_PROVIDED);
            }
        });
        atmRouter.get(routeConstants_1.routeConstants.BALANCE, (req, resp) => {
            //Here we will return an error object if the account don't match
            if (this.accountInToken != req.params.acct) {
                resp.json(errorMessages_1.errorMessages.ACCOUNTS_DONT_MATCH);
            }
            else {
                this.logger.debug("Attempting to get Current Balance for Acct # ", req.params.acct);
                this.atm.getCurrentBalance(req.params.acct).then(result => {
                    //log it 
                    this.logger.debug("Queried Balance for Acct # ", req.params.acct, "Balance", result.currentBalance);
                    resp.json({
                        status: 0,
        ...
        atmRouter.get(routeConstants_1.routeConstants.DEPOSIT, (req, resp) => {
            //Here we will return an error object if the account don't match
            if (this.accountInToken != req.params.acct) {
                ...
                this.atm.deposit(req.params.acct, parseFloat(req.params.amount)).then(result => {
                    //log it 
                    this.logger.debug("Peformed a deposit of  ", req.params.amount, "to account # ", req.params.acct);
                    resp.json({
                        status: 0,
                        accountNumber: result.accountNumber,
                        currentBalance: result.currentBalance
                    });
                }, err => { resp.json(errorMessages_1.errorMessages.ACCOUNT_NOT_FOUND); });
            }
        });
        atmRouter.get(routeConstants_1.routeConstants.WITHDRAW, (req, resp) => {
            //Here we will return an error object if the account don't match
            if (this.accountInToken != req.params.acct) {
                //as the resp.json is a return statement we won't place an else here
                this.logger.info(errorMessages_1.errorMessages.ACCOUNT_NOT_FOUND);
                resp.json(errorMessages_1.errorMessages.ACCOUNTS_DONT_MATCH);
            }
            else {
                this.atm.getCurrentBalance(req.params.acct).then(result => {
                    if (result.currentBalance < req.params.amount) {
                        this.logger.info(errorMessages_1.errorMessages.INSUFFICIENT_FUNDS);
                        resp.json(errorMessages_1.errorMessages.INSUFFICIENT_FUNDS);
                    }
                    else {
                        this.atm.withDraw(req.params.acct, parseFloat(req.params.amount)).then(result => {
                            //log it 
                            this.logger.debug("Peformed a withtrawl of  ", req.params.amount, " from account # ", req.params.acct);
                            resp.json({
                                status: 0,
                                accountNumber: result.accountNumber,
                                currentBalance: result.currentBalance
        ...
        atmRouter.get(routeConstants_1.routeConstants.TRANSACTIONS, (req, resp) => {
            //Here we will return an error object if the account don't match
            if (this.accountInToken != req.params.acct) {
                //as the resp.json is a return statement we won't place an else here
                resp.json(errorMessages_1.errorMessages.ACCOUNTS_DONT_MATCH);
                ...
                }, notFound => { resp.json(errorMessages_1.errorMessages.ACCOUNT_NOT_FOUND); });
            }
        });
